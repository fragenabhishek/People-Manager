...
def _create_person(client, name="John Doe", **overrides):
    ...
    resp = client.post('/api/people', data=json.dumps(data), content_type='application/json')
    return resp
...
def test_get_person_by_id(auth_client):
    create_resp = _create_person(auth_client)
    pid = _json(create_resp)['data']['id']
...
def test_update_person(auth_client):
    ...
    resp = auth_client.put(
        f'/api/people/{pid}',
        data=json.dumps({"name": "Jane Doe", "company": "NewCo"}),
...
def test_add_and_remove_tags(auth_client):
    ...
    resp = auth_client.post(
        ...
        data=json.dumps({"tags": ["mentor", "vip"]}),
        content_type='application/json',
    )
    assert resp.status_code == 200
...
def test_set_and_complete_followup(auth_client):
    ...
    resp = auth_client.put(
        f'/api/people/{pid}/followup',
        data=json.dumps({"date": "2026-03-20", "frequency_days": 7}),
        content_type='application/json',
    )
    assert resp.status_code == 200
    ...
    resp = auth_client.post(
        f'/api/people/{pid}/followup/complete',
        content_type='application/json',
    )
    assert resp.status_code == 200
